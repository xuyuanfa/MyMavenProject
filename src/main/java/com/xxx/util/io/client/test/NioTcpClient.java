...
public class NioTcpClient {
	...
	private final NioBuffer nioBuffer = new NioBuffer();
	...
	final public NioWriteFuture write(byte[] data, ISocketCallBack callback) throws IOException, InterruptedException {
			...
			public void run() {
				...
				try {
					if (getHeadSize() == HEAD_SIZE_8) {
						write("00000000".getBytes("UTF-8"));
					} else if (getHeadSize() == HEAD_SIZE_6) {
						write("000000".getBytes("UTF-8"));
					}
				} catch (IOException e) {
					log.error("", e);
