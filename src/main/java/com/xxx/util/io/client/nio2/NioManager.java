...
/**
 * xuyf:该类是领导者角色，外部单例调用，单例一个领导者已经足够管理；
 * 
 * 两个线程处理NioManager的职能：
 ...
 * INioHandler处理器，NIO客户端中各类操作处理后的响应处理，可设计报文长度读取等；
 */
public class NioManager {
	...
	private static NioManager nioManager = null;
	...
	public static NioManager getInstance() throws IOException{
		if(nioManager == null){
			synchronized (NioManager.class) {
				if(nioManager == null){
					NioManager nioManagerTmp = new NioManager();
					nioManager = nioManagerTmp;
				}
			}
		}
		return nioManager;
	...
	private static void shutdown(ExecutorService executor) {
		// 关闭线程池，不接收新任务，并等待处理待处理任务
		executor.shutdown();
		try {
			// 等待处理完的所有待处理的任务后，关闭线程池
